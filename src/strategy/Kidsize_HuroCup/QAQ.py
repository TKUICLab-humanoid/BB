...
BASKET_SIZE_100_150 =[2116, 899] #sector 111   left side 1978, 899 right side  2140, 961  #投籃時測量的籃框距離方法 #五分投籃時站姿高度看籃框size測距離
FIVE_POINT_HEAD_DEGREE = [1950]# left side 1960 right side  1940   too left-big too right-small #投籃前頭會固定一個角度，並扭腰
THROW_PLUS = 1 #line  0   left side 0 right side  4

# THROW_BALL_POINT = [0,0,1300]   #[0,1,2]=[三分,五分,兩分] #二分走路時測量的籃框距離方法degree #五分走路時站姿高度看籃框size測距離
#                  [size,size,degree] 
...
CORRECT       = [-300,50,0]         #原地踏步修正
LEFT_CORRECT  = [-300,-100,4]       #左旋修正
RIGHT_CORRECT = [-300,200,-4]       #右旋修正
#                  x , y , theta 

CATCH_BALL_LINE = [1700,1650,1530] #forward_degree,slow_degree,backward_degree
TWO_POINT_LINE  = [2350,2180,2450] #forward_degree,slow_degree,backward_degree
...
FIVE_POINT_LINE  = [2300,2180,2440] #forward_degree,slow_degree,backward_degree
...
class Motor_Move():
    ...
    def degree_straight(self,forward_degree,slow_degree,backward_degree) :   #前進後退至可找拿球的距離
        ...
        #     time.sleep(1)
        #     print("頭部水平旋轉調整")
        #     motor.move_head(1,1960,880,880,30)
        #     time.sleep(0.8)
        #     print("伸手準備投籃")
...
class BasketBall():
    ...
    def walk_to_ball(self):
        if CATCH_BALL_LINE[2] <= motor.head_vertical <= CATCH_BALL_LINE[1] :    #到達夾球位置
    ...
    def stratagy_2(self):
        ...
        if not (TWO_POINT_LINE[2] <= motor.head_vertical <= TWO_POINT_LINE[1]) :
        ...
        elif TWO_POINT_LINE[2] <= motor.head_vertical <= TWO_POINT_LINE[1] :
    ...
    def stratagy_3(self):
        ...
        if not (THREE_POINT_LINE[2] <= target.basket_size <= THREE_POINT_LINE[1]) :
        ...
        elif THREE_POINT_LINE[2] <= target.basket_size <= THREE_POINT_LINE[1] :
            if self.throw_ball_action == True :
    ...
    def stratagy_5(self):
        ...
        if not (THREE_POINT_LINE[2] <= target.basket_size <= THREE_POINT_LINE[1]) :
            ...
            if self.throw_ball_action == True :
                ...
                if target.basket_x != 0 and abs(target.basket_x-160) > 2 :

                    # if abs(target.basket_x-160) > 2:
                    rospy.logdebug(f"target.basket_size = {target.basket_size}")
                    motor.WaistFix(target.basket_x,160)
                    rospy.logdebug("abs(target.basket_x-160) = {abs(target.basket_x-160)}")
                ...
                elif target.basket_x != 0 and abs(target.basket_x-160) <= 2 :

...
if __name__ == '__main__' :
    ...
    five_point_degree = [1957]# left side 1960 right side  1940   too left-big too right-small
    throw_plus = 6 #line  0   left side 0 right side  4

    throw_ball_point = [910,950,1850] #投籃未寫 #strength left 1054 right 1156
    #                    size,degree
    ...
    correct       = [-300,0,0]
    left_correct  = [-350,0,5]
    right_correct = [-350,0,-6]
    #                  x , y , theta   
            ...
            if send.is_start==True :
                ...
                if step == 'begin':
                    ...
                    if target.ball_size <= 350 :
                        motor.view_move(2428,1608,1800,1200,45,0.05)                 
                        target.ball_parameter()  

                    elif target.ball_size > 350 :
                        motor.head_horizon_flag = 1
                    ...
                    if target.ball_size < 300 : #qwerty
                        print("miss ball")      #qwerty
                        step = "find_ball"      #qwerty
                    elif abs(target.ball_x - 160) > 6  or abs(target.ball_y - 120) > 8 :
                        target.ball_parameter()
                        ...
                        if (ball_catch_size[0]+10 <= motor.head_vertical <ball_catch_size[0]+120) and  (1698 <= motor.head_horizon <=2398):
                        ...
                        # elif (ball_catch_size[0]-35 < motor.head_vertical <ball_catch_size[0]+10) and  (1698 <= motor.head_horizon <=2398):
                        #     motor.temp = motor.head_horizon
                        #     motor.move_head(1,1850,880,880,50) #1748
                        #     time.sleep(0.1)
                    ...
                    time.sleep(3)
                    step = 'find_basekt'#@@@@@@@@@@@@@@@@@@
                    if motor.directly == True and sw == 1:
                        step = 'basket_trace'  #qwerty


                elif step == 'find_basekt' :#@@@@@@@@@@@@@@@@@@
                    target.basket_parameter()
                    if target.basket_size < 400 :
                        print("find_basket",target.basket_size)
                        motor.view_move(2748,1548,2048,1948,40,0.04)
                        target.basket_parameter()

                    elif target.basket_size > 400 :
                            step = 'basket_trace'#@@@@@@@@@@@@@@@@@@
                            time.sleep(1)
                            target.basket_parameter()
                        ...
                        if target.basket_size < 400 and  motor.directly == False: #qwerty
                            print("miss ball")        #qwerty
                            step = "find_basekt"     #qwerty
                        
                        elif (abs(target.basket_x - 160) > 12  or abs(target.basket_y - 120) > 9)  and motor.directly == False:
                            target.basket_parameter() 
                        ...
                        if abs(motor.head_horizon-2048) > 80 :#!!!!
                            ...
                            if sw == 2:
                                motor.body_trace_basket_straight_2(throw_ball_point[sw],basket_error[sw]) #接近匡catch=tru
                                if motor.stop == True :
                                    print("step is ",step)#@@@@@@@@@@@@@@@@@@@@@@@@
                                    target.basket_parameter()
                                    time.sleep(1)

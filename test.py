...
CORRECT       = [ -600, -100, 0]        #原地踏步修正
LEFT_CORRECT  = [ -800, -1000, 4]       #左旋修正
RIGHT_CORRECT = [-600, 1060, -4]       #右旋修正
#                 x , y , theta 
...
CATCH_BALL_LINE = [1945, 1775, 1755]            # slow_degree,stop_degree,backward_degree
...
FIVE_POINT_LINE  = [2400, 2650, 2800, 3500]        # forward_slow_size < forward_stop_size < backward_stop_size < backward_slow_size 
#注意 size數值調越大會離籃框越近！！！
...
class BasketBall():
    ...
    def main(self):
        if send.is_start: #send.Web 
            ...
            elif send.DIOValue == 10:        #ball size 下上下下
                self.draw()
    ...
    def begin(self):
        ...
        motor.move_head(2,1800,880,880,50) #請動動作(擺頭)
        send.sendBodySector(128) 
        time.sleep(0.05)
    ...
    def find_ball(self):
        ...
        if target.ball_size <= 300:   #球在視野中太小
            ...
            motor.view_search(2528, 1568, 1800, 1200, 40, 0.05)
            rospy.logdebug(f'target.ball_x = {target.ball_x},target.ball_y = {target.ball_y},target.ball_size = {target.ball_size}')
            target.ball_parameter() 
            ...
            if abs(target.ball_x - 160) > 8  or abs(target.ball_y - 120) > 8:  #讓球在畫面中心
            ...
            elif (2000 <= target.ball_size <= 3500) and (abs(motor.head_horizon-2048) <= 450):
                ...
                motor.move_head(1, 2020, 880, 880, 50)
    ...
    def start_gait(self):
        ...
        if (motor.head_vertical <= 1900) or ((motor.head_vertical < 1445) and  (1698 >= motor.head_horizon or motor.head_horizon >= 2398)): #????
    ...
    def walk_to_ball(self):
        ...
        if (CATCH_BALL_LINE[2] <= motor.head_vertical <= CATCH_BALL_LINE[1]) and (abs(motor.head_horizon-2048) <= 100):  #到達夾球位置
            ...
            if target.ball_size >= 15000:
                motor.MoveContinuous(-1000+CORRECT[0],0+CORRECT[1],0+CORRECT[2],300,300,2)
                rospy.loginfo(f'球太大->後退')

            elif abs(motor.head_horizon-2048) > 100:
        ...
        if target.basket_size < 2000:
            ...
            if self.sw == 0:
                ...
                if abs(target.basket_x - 160) > 6  or abs(target.basket_y - 120) > 8:  #讓球在畫面中心
                    ...
                    if not self.shoot_directly:
                        rospy.loginfo(f'5分球')
                        motor.bodyauto_close(1)
                        time.sleep(1)
                        self.step = 'stratagy_5'
                    else:
                        rospy.loginfo(f'5分球直接投')
                        time.sleep(3)
    ...
    def stratagy_2(self):
        ...
        if not self.ready_shoot:
            ...
            if abs(motor.head_horizon - 2048) > 200:
            ...
            elif (TWO_POINT_LINE[2] <= motor.head_vertical <= TWO_POINT_LINE[1]): #and (abs(motor.head_horizon - 2048) <= 200):
                self.ready_shoot = True
                ...
                time.sleep(1)
                rospy.logdebug(f'頭部水平旋轉調整')
                motor.move_head(1, 1900, 880, 880, 30)
                time.sleep(0.8)
    ...
    def stratagy_3(self):
        ...
        if not self.ready_shoot:  
            ...
            elif (THREE_POINT_LINE[1] <= motor.corrected_size <= THREE_POINT_LINE[2]): #and (abs(motor.head_horizon - 2048) <= 50): 

                self.ready_shoot = True
                rospy.loginfo(f'到達可投籃大小 STOP!!, motor.corrected_size = {motor.corrected_size}')
                motor.bodyauto_close(0)
                target.basket_parameter()
                time.sleep(3)
                rospy.logdebug(f'三分球動作預備')
                rospy.logdebug(f'頭部水平旋轉調整')
                motor.move_head(1, 1950, 880, 880, 30)
                time.sleep(0.8)
                rospy.logdebug(f'伸手準備投籃')
                send.sendBodySector(470)
                time.sleep(5)
    ...
    def stratagy_5(self):
        ...
        if not self.ready_shoot:
            ...
            elif (FIVE_POINT_LINE[1] <= motor.corrected_size <= FIVE_POINT_LINE[2]) and (abs(motor.head_horizon - 2048) <= 50):
                self.ready_shoot = True
                rospy.loginfo(f'到達可投籃大小 STOP!!, motor.corrected_size = {motor.corrected_size}')
                motor.bodyauto_close(0)
                target.basket_parameter()
...
class TargetLocation():
    ...
    def ball_parameter(self):   #利用色模建籃球
        ...
        for j in range (self.color_mask_subject_orange):   #將所有看到的橘色物件編號
            if 10000 > send.color_mask_subject_X [0][j] > 10 and 230 > send.color_mask_subject_Y [0][j] > 10 and send.color_mask_subject_size [0][j] > 10:
    ...
    def basket_parameter(self): #利用色模建籃框
        ...
        for j in range (self.color_mask_subject_red):     #將所有看到的紅色物件編號
            if send.color_mask_subject_size [5][j] > 1000:

                if  80000 > send.color_mask_subject_size [5][j] > self.basket_size:  #用大小過濾物件(濾雜訊)
...
class MotorMove():
    ...
    ######################################## ball_trace_straight 副函式 ########################################
        if self.head_vertical > slow_degree:  #大前進s
            self.MoveContinuous(3500+CORRECT[0], 0+CORRECT[1], 0+CORRECT[2], 300, 300, 2) 
            rospy.loginfo(f'大前進, self.head_vertical= {self.head_vertical}')

        elif stop_degree < self.head_vertical < slow_degree:  #進入減速範圍
            self.MoveContinuous(2500+CORRECT[0], 0+CORRECT[1], 0+CORRECT[2], 300, 300, 2)
    ...
    ######################################## degree_straight 副函式 ######################################## 
        ...
        if self.head_vertical > slow_degree:  #大前進
            self.MoveContinuous(3000+CORRECT[0], 0+CORRECT[1], 0+CORRECT[2], 300, 300, 2) 
            rospy.loginfo(f'大前進, self.head_vertical =  {self.head_vertical}')
            time.sleep(0.05)

        elif stop_degree < self.head_vertical < slow_degree :  #進入減速範圍
            self.MoveContinuous(1500+CORRECT[0], 0+CORRECT[1], 0+CORRECT[2], 300, 300, 2)
            rospy.loginfo(f'進入減速範圍, self.head_vertical =  {self.head_vertical}')
            time.sleep(0.05)
    ...
    ########################################  size_straight副函式 ########################################
        ...
        if motor.corrected_size > backward_slow_size:                         #大後退
            ...
            self.MoveContinuous(-1000+CORRECT[0], 0+CORRECT[1], 0+CORRECT[2], 300, 300, 2)
 ...
 ########################################  size_IMU_correct 副函式 ########################################   
    ...
    def basket_distance(self, six, nine):   #利用60與90公分距離測出的籃框面積推得當前機器人與籃框的距離->self.distance_new
        if target.basket_size != 0:
            ...
            rospy.loginfo(f'Distance_100 = {sixty_distance}')
            rospy.loginfo(f'Distance_1500 = {ninty_distance}')

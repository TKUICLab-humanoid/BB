...
FIVE_POINT_HEAD_DEGREE = [1950]         #投籃前頭會固定一個角度，並扭腰
THROW_PLUS = 1 
BALL_CATCH_SIZE =[1900] 
ROBBOT_HIGT = 87


#======================================================================================

CORRECT       = [-1100,-000,1]          #原地踏步修正
LEFT_CORRECT  = [-1300,-400,4]          #左旋修正
RIGHT_CORRECT = [-1300,-300,-3]         #右旋修正
#                  x , y , theta 

CATCH_BALL_LINE = [2150,2250,2350] #forward_degree,slow_degree,backward_degree
TWO_POINT_LINE  = [2180,2350,2450] #forward_degree,slow_degree,backward_degree
THREE_POINT_LINE = [4950,5050,5300] #forward_degree,slow_degree,backward_degree
#three_point_line = [2000,1930,1900] #forward_degree,slow_degree,backward_degree
FIVE_POINT_LINE  = [2370,2470,2650] #forward_degree,slow_degree,backward_degree
...
class Target_Location():
    ...
    def ball_parameter(self):    #利用色模建籃球
        ...
        for j in range (self.color_mask_subject_orange):   #將所有看到的橘色物件編號
            if 900 > send.color_mask_subject_X [0][j] > 10 and 1000 > send.color_mask_subject_Y [0][j] > 10:
...
####################################### view search #######################################
    def view_search(self, right_place, left_place, up_place, down_place, speed, delay):   
        rospy.loginfo(f'motor.reg =  {bb.reg}')
        if bb.reg > 0:
            turn_order = [3, 4, 1, 2]
        else:
            turn_order = [1, 4, 3, 2]
...
####################################### view search #######################################
    ...
    def degree_straight(self,forward_degree,slow_degree,backward_degree) :   #前進後退至可找拿球的距離
        ...
        if self.head_vertical < forward_degree :  #大前進
            motor.MoveContinuous(3500+CORRECT[0],0+CORRECT[1],0+CORRECT[2],100,100,2) 
            rospy.loginfo(f"大前進,self.head_vertical = {self.head_vertical}")
            time.sleep(0.05)
    ...
    def MoveContinuous(self ,ExpX ,ExpY ,ExpTheta ,AddX ,AddY ,AddTheta) :  #步態移動的馬達緩衝(調整距離:Now_X與Now_Y為現在要移動的x量與現在要移動的y量)
        rospy.loginfo(f"IMU = {send.imu_value_Pitch}") 
...
class BasketBall():
    ...
    def begin(self):
        ####################################### switch #######################################
        # send.sendBodySector(894)
        send.sendSensorReset(1, 1, 1)
        send.sendBodySector(357)
    ...
    ######################################## switch #######################################
        ...
        motor.move_head(2,1500,880,880,50) #請動動作(擺頭)
    ...
    def find_basket(self):
        ...
        if target.basket_size < 2000 :
            ...
            motor.view_search(2457,1698,2577,2048,50,0.04)
    ...
    def stratagy_3(self):
        ...
        if self.throw_ball_action:
            if abs(motor.head_horizon-2048) > 100  :
            ...
            elif THREE_POINT_LINE[0] <= target.basket_size <= THREE_POINT_LINE[1] :
                ...
                rospy.logdebug(f"頭部水平旋轉調整")
                motor.move_head(1,1930,880,880,30)
                time.sleep(0.8)
    ...
    def stand(self):
        if send.DIOValue == 11:           #catch ball point  上上下下 
            ...
            rospy.loginfo(f'Head_vertical = {motor.head_vertical}')
        elif send.DIOValue == 11:        #ball size 上下下下
            self.drawline()
            target.ball_parameter()
            motor.trace_revise(target.basket_x, target.basket_y, 35)
